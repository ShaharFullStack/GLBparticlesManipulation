// We// WebGPU Compute Shaders

  
  
  // Bindings for data bufferse Shader
//
...
    
    ...
    let targetForce = (attractionTarget - particle.position) * uniforms.attraction;    // Mouse Interaction
    var mouseForce = vec3<f32>(0.0);
...
  
  ...
  struct FiberUniforms {
    ...
    deltaTime: f32,
    mousePos: vec3<f32>,
    mouseInfluenceRadius: f32,
    enableMouseConnections: f32,
    particleConnectionDistance: f32
  };  // Bindings for data buffers
...
  
  async initializeFibers() {
    ...
    // Create a random set of connections between particles
    for (let i = 0; i < Math.min(this.particleCount, 1000) && fiberIndex < this.maxFibers; i++) {
      for (let j = i + 1; j < Math.min(this.particleCount, 1000) && fiberIndex < this.maxFibers; j++) {
        // Connect nearby particles based on distance
        if (Math.random() < 0.02) { // Slightly higher probability for more connections
          ...
          fiberData[offset + 3] = 3.0; // resting length (increased for better visibility)
    ...
    if (this.fibersEnabled && this.device) {
      // Implement dynamic fiber creation based on particle proximity
      this.updateDynamicConnections();
    }
  }

  updateDynamicConnections() {
    // Get current particle positions to calculate dynamic connections
    if (!this.particles || !this.device) return;
    
    // Create a simplified version for CPU-side calculation
    // In a full implementation, this could be moved to GPU compute shader
    const connectionDistance = 8.0; // Distance threshold for creating connections
    const maxConnectionsPerParticle = 6; // Limit connections to prevent performance issues
    
    // This is a simplified approach - for better performance, 
    // the connection logic should be moved to a compute shader
    console.log('Updating dynamic fiber connections...');
  }
